...
with a as(
    ...
    FROM
        ...
        inner join `opscenter.networktest.GeoIP_ISP_Range` gir ON NET.IP_FROM_STRING(nt.ClientIP) < gIr.end_ip_byte
        AND NET.IP_FROM_STRING(nt.ClientIP) > gIr.start_ip_byte
    WHERE
        nt.Diagtype = 4
        and gcr.country_iso_code = 'MY'
        AND nt.Time > "2021-03-31 09:00:00" # AND nt.Time <= "2021-03-31 14:30:00"
        AND nt.Average != 0
        and nt.Average > 300
)
select
    ...
    isp,
    asn,
    count(1) as diag_counts,
    avg(Average) as ave_latency
from
    a
group by
    country_iso_code,
    isp,
    asn # group by nt.UserID
    # AND nt.Average < 1000
order by
    4 desc