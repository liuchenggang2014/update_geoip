...
WHERE
  ...
  and NET.IP_FROM_STRING(end_IP) > NET.IP_FROM_STRING('61.18.36.61') -- search by asn number 
SELECT
  a.ClientIP,
  b.start_IP,
  b.end_IP,
  ...
  a.time
FROM
  `opscenter.network_diagnosis_asia.htzx_asia` as a
  inner join `opscenter.networktest.GeoIP_ISP_Range` as b on NET.IP_FROM_STRING(a.ClientIP) between b.start_ip_byte
  and b.end_ip_byte
where
  b.asn = '9908'
