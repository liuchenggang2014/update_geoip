...
SELECT
  DISTINCT TIMESTAMP_SECONDS(15*60 * DIV(UNIX_SECONDS(PARSE_TIMESTAMP('%F %H:%M:%S', time)), 15*60)) timekey,
  PERCENTILE_CONT(Average,
    0.9) OVER(PARTITION BY TIMESTAMP_SECONDS(15*60 * DIV(UNIX_SECONDS(PARSE_TIMESTAMP('%F %H:%M:%S', time)), 15*60))) AS p90
FROM
