...
CREATE TEMP FUNCTION cidrToRange(CIDR STRING) RETURNS STRUCT < start_IP STRING,
end_IP STRING > LANGUAGE js AS """
...
or replace table `opscenter.networktest.GeoIP_ISP_Range` as (
    SELECT
        network,
        IP_range.*,
        NET.SAFE_IP_FROM_STRING(IP_range.start_IP) as start_ip_byte,
        NET.SAFE_IP_FROM_STRING(IP_range.end_ip) as end_ip_byte,
        isp,
        asn,
        aso
    FROM
...
or replace table `opscenter.networktest.GeoIP_Country_Range` as (
    SELECT
        ...
        country_iso_code
    FROM
        `opscenter.networktest.GeoIP_Country`,
        UNNEST([cidrToRange(network)]) IP_range
);